...
{
  ldpp_dout(this, 0) << "shr::  send_partial_response" << dendl;
  if (! sent_header) {
    ldpp_dout(this, 0) << "shr::  send_partial_response:: if" << dendl;
    if (op_ret)
    set_req_state_err(s, op_ret);
    dump_errno(s);
    ...
    dump_start(s);
    if (op_ret == 0) {
      ldpp_dout(this, 0) << "shr::  send_partial_response:: if ::if" << dendl;
      s->formatter->open_object_section_in_ns("CopyObjectResult", XMLNS_AWS_S3);
    }
    sent_header = true;
  } else {
    ldpp_dout(this, 0) << "shr::  send_partial_response:: else" << dendl;
